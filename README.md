...
Alternatively, you can run a `Docker` image:
...
Open you browser and navigate to `localhost:8080`, enter the password "securewecret" :) and create a new Livebook or upload a file or paste a URL pointing to a ".livebook.livemd".

Thee file is: **[learn-elixir-on-livebook.livemd]** and [this is the URL](https://github.com/dwyl/learn-elixir/blob/main/learn-elixir-on-livebook.livemd)

- If you have `Livebook` installed on your machine, this is the easiest experience as you will use a more powerful computer (yours 🥳). Just click on the button below (or fork and run it):
...
- if you don't have `Docker` nor `Elixir` and `Livebook` installed, you can run a remote version in the cloud. Follow this!
